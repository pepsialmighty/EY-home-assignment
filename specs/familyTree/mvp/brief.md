...
### CI/CD & Packaging
- Dockerize the application (frontend + backend) for local self-hosting via `docker compose up`
- GitHub Actions CI pipeline runs tests on every push to `main`
- GitHub Actions CD pipeline deploys both services to Fly.io automatically on merge to `main` (after tests pass)
...
- **Frontend:** React 18 + TypeScript + Vite + TanStack Query + Tailwind CSS, served at `http://localhost:3000`
- **Backend:** Node.js + TypeScript + Express + node:sqlite (built-in Node 22+), served at `http://localhost:5000`
- **Database:** SQLite (via `node:sqlite` built-in module)
